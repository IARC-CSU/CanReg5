...
text <- canreg_import_txt("Background.txt", folder = report_text)
if (!grepl("NA\n", text)) {
...
text <- canreg_import_txt("Population.txt", folder = report_text)
if (!grepl("NA\n", text)) {
  doc <- addParagraph(doc,  text) 
} 
doc <- addParagraph(doc, "\n")

dims <- attr( jpeg::readJPEG (paste0(report_text, "map_general.jpg")), "dim" )
doc <- addImage(doc, paste0(report_text, "map_general.jpg"),width=3,height=3*dims[1]/dims[2],par.properties = parProperties(text.align = "left"))
...
text <- canreg_import_txt("Methods.txt", folder = report_text)
if (!grepl("NA\n", text)) {
  doc <- addParagraph(doc,  text) 
} 
...
canreg_output(output_type = "png", filename = paste0(tempdir(), "\\temp_graph"),landscape = landscape,list_graph = FALSE,
